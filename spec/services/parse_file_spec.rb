...
describe 'Parse file' do
  before(:each) do
    file = File.open('./spec/resources/testFile.txt')
  ...
  it 'Success Payment' do
    ...
    expect(payment.id).to eq('9985b3c3db2c42ffa481fa7daccacfdf')
    ...
    expect(payment.client.id).to eq('aee603927f1a4a719a5f02656803c553')
  end

  it 'Succes payment transactions' do
    transactions = @payments.first.transactions
    first_transaction = transactions.detect { |tr| tr.id = '4d413a2fa47b41d0b33b51416dede498' }

    expect(transactions.size).to eq(21)
    expect(first_transaction.id).to eq('4d413a2fa47b41d0b33b51416dede498')
  ...
  it 'Succes payment discounts' do
    ...
    expect(first_discount.id).to eq('ab64b1b344fa4db18c3280547ade35ef')
